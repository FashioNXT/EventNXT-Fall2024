class ReferralsController < ApplicationController
  ...
  def referral_creation
    ...
    @guest = Guest.find_by(rsvp_link: random_code)
    if @guest
      @referral = Referral.find_or_create_by(event_id: @guest.event_id,
        guest_id: @guest.id, email: @guest.email, name: "#{@guest.first_name} #{@guest.last_name}", referred: friend_email, ref_code: @guest.id)
      @referral.save
  ...
  def referral_params
    params.require(:referral).permit(:event_id, :guest_id, :email, :name,
      :referred, :status, :tickets, :amount, :reward_method, :reward_input, :ref_code, :reward_value)
  end
end
