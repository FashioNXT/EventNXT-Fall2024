...
class Guest < ApplicationRecord
  ...
  def self.new_guest(attributes = {})
    ...
    Guest.new(attributes) # creates new guest
    # return guest
  end
  ...
  def allocated_seats_not_exceed_total
    ...
    seat = event.seats.find_by(category:)
    return unless seat.present?

    existing_guest = event.guests.find_by(id:)
    if existing_guest.present?
      total_allocated_seats = event.guests.where(category:).sum(:alloted_seats) - existing_guest.alloted_seats.to_i
      total_commited_seats = event.guests.where(category:).sum(:commited_seats) - existing_guest.commited_seats.to_i
    else
      total_allocated_seats = event.guests.where(category:).sum(:alloted_seats)
      total_commited_seats = event.guests.where(category:).sum(:commited_seats)
    end
    ...
    puts event.guests.where(category:).sum(:alloted_seats)
    ...
    if (total_allocated_seats + alloted_seats.to_i) > seat.total_count
      errors.add(:alloted_seats,
                 "cannot exceed the total allocated seats (#{remaining_allocated_seats} remaining) for the category #{category}")
    end

    return unless (total_commited_seats + commited_seats.to_i) > total_allocated_seats + alloted_seats.to_i

    errors.add(:commited_seats,
               "cannot exceed the total allocated seats (#{remaining_committed_seats} remaining) for the category #{category}")
  end
